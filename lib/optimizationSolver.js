...
orderPressure: {
incoming: result.pressure?.incoming || 0,
outgoing: result.pressure?.outgoing || 0,
combinationCount: result.pressure?.combinationCount || 0
},
...
const buildingPressure = new Map(buildingResults.map((result) => [result.building, {
incoming: 0,
outgoing: 0,
combinationCount: result.keptCombinationsBeforeLimit || result.generatedCombinations || result.combinations.length
}]));
for (const result of buildingResults) {
for (const combination of result.combinations) {
const requiredMake = combination.requiredMake || requirementsFit(combination.sell, model);
let outgoingAmount = 0;
for (const [requiredKey, amount] of requiredMake.entries()) {
const requiredProduct = model.productsByKey.get(requiredKey);
if (!requiredProduct) continue;
const requiredBuilding = getBuildingName(requiredProduct);
if (requiredBuilding === result.building) continue;
outgoingAmount += amount;
const pressure = buildingPressure.get(requiredBuilding);
if (pressure) pressure.incoming += amount * Math.max(combination.value, 1);
}
const ownPressure = buildingPressure.get(result.building);
if (ownPressure) ownPressure.outgoing += outgoingAmount * Math.max(combination.value, 1);
}
}
...
pressure: buildingPressure.get(result.building) || { incoming: 0, outgoing: 0, combinationCount: result.combinations.length }
}))
.sort((a, b) =>
b.pressure.incoming - a.pressure.incoming ||
b.pressure.combinationCount - a.pressure.combinationCount ||
b.pressure.outgoing - a.pressure.outgoing ||
b.upperValue - a.upperValue ||
...
combinationsEvaluated: stats.combinationsEvaluated,
prunedByDominance: stats.prunedByDominance,
prunedByStateDominance: stats.prunedByStateDominance,
prunedBeforeRecursionByMaterialFlow: stats.prunedBeforeRecursionByMaterialFlow,
