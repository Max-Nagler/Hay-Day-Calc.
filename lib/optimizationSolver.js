...
export function solveOptimizationModel(model) {
  ...
  const itemsByBuilding = new Map();
  for (const item of allItems) {
    const building = getBuildingName(item.product);
    if (!itemsByBuilding.has(building)) itemsByBuilding.set(building, []);
    itemsByBuilding.get(building).push(item);
  }

  const selectedItems = new Map();
  for (const [building, buildingItems] of itemsByBuilding.entries()) {
    const slotLimit = getSlotsForBuilding(building, model.settings);
    const perBuildingLimit = Math.max(slotLimit * 4, 12);

    for (const item of buildingItems
      .sort((a, b) => b.value - a.value || b.efficiency - a.efficiency)
      .slice(0, perBuildingLimit)) {
      selectedItems.set(item.product.key, item);
    }

    for (const item of buildingItems
      .sort((a, b) => b.efficiency - a.efficiency || b.value - a.value)
      .slice(0, perBuildingLimit)) {
      selectedItems.set(item.product.key, item);
    }
  }
  ...
  const items = Array.from(selectedItems.values())
    // Reihenfolge nach Gebäude gruppieren: Dadurch werden Gebäude-Kombinationen
    // früher geschlossen und das Pruning arbeitet innerhalb des Vercel-Limits besser.
    .sort((a, b) =>
      getBuildingName(a.product).localeCompare(getBuildingName(b.product), "de") ||
      b.value - a.value ||
  ...
  return {
    solverStatus: visitedNodes > maxNodes ? "feasible_server_node_limited" : "optimal",
