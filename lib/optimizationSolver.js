...
function selectLocalCombinations(combinations, limit) {
const nonEmpty = combinations.filter((combo) => combo.slots > 0);
if (nonEmpty.length <= limit) {
return {
combinations: nonEmpty,
selection: {
selectedByGlobalScore: nonEmpty.length,
selectedByValue: 0,
selectedByLowMaterialPressure: 0,
selectedByDiversity: 0
}
};
}
const selected = new Map();
const byGlobalScore = [...nonEmpty].sort((a, b) =>
(b.globalScore || b.value) - (a.globalScore || a.value) ||
b.value - a.value ||
(a.materialPressure || 0) - (b.materialPressure || 0)
);
const byValue = [...nonEmpty].sort((a, b) =>
b.value - a.value ||
(a.materialPressure || 0) - (b.materialPressure || 0) ||
a.minutes - b.minutes
);
const byLowMaterialPressure = [...nonEmpty].sort((a, b) =>
(a.materialPressure || 0) - (b.materialPressure || 0) ||
b.value - a.value ||
a.minutes - b.minutes
);
const selectedByGlobalScore = addUniqueCombinations(selected, byGlobalScore, Math.ceil(limit * 0.5));
const beforeValue = selected.size;
addUniqueCombinations(selected, byValue, Math.ceil(limit * 0.75));
const selectedByValue = selected.size - beforeValue;
const beforeLowPressure = selected.size;
addUniqueCombinations(selected, byLowMaterialPressure, Math.ceil(limit * 0.9));
const selectedByLowMaterialPressure = selected.size - beforeLowPressure;
const bestByProfile = new Map();
for (const combination of nonEmpty) {
const profile = combinationProfileSignature(combination);
const current = bestByProfile.get(profile);
if (
!current ||
(combination.globalScore || combination.value) > (current.globalScore || current.value) ||
((combination.globalScore || combination.value) === (current.globalScore || current.value) && combination.value > current.value)
) {
...
addUniqueCombinations(selected, byGlobalScore, limit);
}
return {
...
const localSelection = selectLocalCombinations(dominanceResult.combinations, maxCombinationsPerBuilding);
...
beamExact: stats.beamExact,
beamDroppedStates: stats.beamDroppedStates,
beamSelection: stats.beamSelection,
combinationsEvaluated: stats.combinationsEvaluated,
prunedByDominance: stats.prunedByDominance,
