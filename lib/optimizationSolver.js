...
.slice(0, model.settings.solverMode === "proof" ? Math.max(slotLimit * 4, 18) : model.settings.solverMode === "balanced" ? Math.max(slotLimit * 3, 12) : Math.max(slotLimit * 2, 8));
...
const maxCombinationsPerBuilding = model.settings.solverMode === "proof" ? 40 : model.settings.solverMode === "balanced" ? 20 : 12;
...
solverMode: model.settings.solverMode || "fast",
...
const isProofMode = model.settings.solverMode === "proof";
const maxRuntimeMs = safeNumber(model.settings.maxRuntimeMs, isProofMode ? 25000 : 3500);
const beamWidth = safeNumber(model.settings.beamWidth, isProofMode ? 2000 : 80);
