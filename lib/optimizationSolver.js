...
const make = requirementsFit(selection, model);
const usage = buildUsageFromMake(make, model.productsByKey);
let pressure = 0;

for (const [building, slots] of usage.buildingSlots.entries()) {
const slotCapacity = getSlotsForBuilding(building, model.settings);
const minutes = usage.buildingMinutes.get(building) || 0;
const timeLimit = getTimeLimitMinutes(model.settings);
const slotRatio = slots / Math.max(slotCapacity, 1);
const minuteRatio = minutes / Math.max(timeLimit, 1);

pressure += slots * 80;
pressure += minutes * 0.25;
pressure += Math.max(slotRatio - 0.5, 0) * 500;
pressure += Math.max(minuteRatio - 0.5, 0) * 300;
}
...
.slice(0, Math.max(slotLimit * 4, 12));
...
const combinations = dominanceResult.combinations.slice(0, 45);
...
const maxNodes = 70000;
const startedAt = Date.now();
const maxRuntimeMs = 22000;
...
seedGreedyFeasibleSolution((a, b) => a.building.localeCompare(b.building, "de"));
seedGreedyFeasibleSolution(null, (a, b) => b.value - a.value || (a.materialPressure || 0) - (b.materialPressure || 0));
seedGreedyFeasibleSolution(null, (a, b) => (a.materialPressure || 0) - (b.materialPressure || 0) || b.value - a.value);
...
const isFeasiblePrefix = evaluateSelection(selection, selectedCombinations, currentValue);
if (!isFeasiblePrefix) return;
...
const nextRequiredMake = requirementsFit(nextSelection, model);
const nextUsage = buildUsageFromMake(nextRequiredMake, productsByKey);
if (!usageFits(nextUsage.buildingMinutes, nextUsage.buildingSlots, model.settings)) {
...
if (stats.nodesVisited > maxNodes || stoppedByTime) break;
}
}
...
return {
solverStatus: stoppedByTime ? "feasible_combination_time_limited" : stats.nodesVisited > maxNodes ? "feasible_combination_node_limited" : "optimal",
