...
.slice(0, Math.max(slotLimit * 3, 10));
...
const nonEmptyCombinations = dominanceResult.combinations.filter((combo) => combo.slots > 0).slice(0, 30);
...
const maxRuntimeMs = safeNumber(model.settings.maxRuntimeMs, 8000);
const beamWidth = safeNumber(model.settings.beamWidth, 350);
