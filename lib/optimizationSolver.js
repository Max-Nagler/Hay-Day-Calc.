...
function probeStateArchiveSignature(state) {
const valueBucket = Math.floor((state.value || 0) / 100);
return `${selectionSignature(state.selection || new Map())}::${usageProfileSignature(state)}::${valueBucket}`;
}
...
function selectBeamStates(states, beamWidth, scoreFn, options = {}) {
...
const diversityRatio = Math.max(0.2, Math.min(0.45, safeNumber(options.diversityRatio, 0.2)));
const scoreRatio = Math.max(0.4, Math.min(0.65, 0.8 - diversityRatio));
const valueRatio = Math.max(scoreRatio, Math.min(0.85, scoreRatio + 0.2));
...
const selectedByScore = addUniqueStates(selected, byScore, Math.ceil(beamWidth * scoreRatio));
const selectedByValue = addUniqueStates(selected, byValue, Math.ceil(beamWidth * valueRatio));
...
const topStateArchiveLimit = Math.max(safeNumber(options.topStateArchiveLimit, 0), 0);
const topStateArchive = [];
const topStateArchiveSignatures = new Set();
let topStateArchiveCandidates = 0;
...
function rememberTopStates(candidates) {
if (!topStateArchiveLimit || !Array.isArray(candidates) || !candidates.length) return;
topStateArchiveCandidates += candidates.length;
for (const state of candidates) {
const archiveSignature = probeStateArchiveSignature(state);
if (topStateArchiveSignatures.has(archiveSignature)) continue;
topStateArchiveSignatures.add(archiveSignature);
topStateArchive.push({ ...state, archiveSignature });
}
topStateArchive.sort((a, b) => b.value - a.value || stateScore(b) - stateScore(a));
const unique = new Map();
for (const state of topStateArchive) {
if (unique.size >= topStateArchiveLimit) break;
const signature = state.archiveSignature || probeStateArchiveSignature(state);
if (unique.has(signature)) continue;
unique.set(signature, state);
}
topStateArchive.length = 0;
topStateArchive.push(...unique.values());
topStateArchiveSignatures.clear();
for (const state of topStateArchive) {
topStateArchiveSignatures.add(state.archiveSignature || probeStateArchiveSignature(state));
}
}
...
const pressureBeforeTrim = Math.max(0, statesBeforeTrim - beamWidth);
const adaptiveDiversityRatio = pressureBeforeTrim > beamWidth * 2
? 0.35
: pressureBeforeTrim > beamWidth
? 0.3
: pressureBeforeTrim > 0
? 0.25
: 0.2;
let beamSelection = {
...
selectedByValue: 0,
selectedByDiversity: 0,
diversityRatio: adaptiveDiversityRatio
};
if (nextStates.length > beamWidth) {
const selectionResult = selectBeamStates(nextStates, beamWidth, stateScore, { diversityRatio: adaptiveDiversityRatio });
nextStates = selectionResult.states;
beamSelection = selectionResult.selection;
beamSelection.diversityRatio = adaptiveDiversityRatio;
stats.beamExact = false;
...
if (adaptiveDiversityRatio > 0.2) stats.beamSelection.adaptiveDiversityLevels += 1;
}
if (statesBeforeTrim > beamWidth) {
stats.trimFeedback[buildingResult.building] = {
statesBeforeTrim,
...
.concat(topStateArchive)
.sort((a, b) => b.value - a.value)
.filter((state, index, array) => array.findIndex((candidate) => (candidate.archiveSignature || probeStateArchiveSignature(candidate)) === (state.archiveSignature || probeStateArchiveSignature(state))) === index)
.slice(0, Math.min(50, Math.max(states.length, topStateArchive.length)));
return {
...
topStates,
topStateArchiveDebug: {
enabled: topStateArchiveLimit > 0,
limit: topStateArchiveLimit,
candidatesSeen: topStateArchiveCandidates,
archiveSize: topStateArchive.length,
returnedTopStates: topStates.length,
diversitySignatures: new Set(topStates.map((state) => state.archiveSignature || probeStateArchiveSignature(state))).size
},
stats,
maxRuntimeMs,
...
topStateArchive: probeResult.topStateArchiveDebug,
...
function repairGroupSignature(group) {
return [...group].sort((a, b) => a.localeCompare(b, "de")).join("|");
}
function repairGroupsOverlap(left, right) {
const rightSet = new Set(right);
return left.some((building) => rightSet.has(building));
}
...
function runLocalRepair(model, buildingResults, baseResult, options = {}) {
const startedAt = Date.now();
const defaultRuntimeMs = Math.max(500, Math.min(3500, Math.floor(Math.max(safeNumber(model.settings.maxRuntimeMs, 3500), 250) * 0.22)));
const maxRuntimeMs = Math.max(250, safeNumber(options.maxRuntimeMs, defaultRuntimeMs));
const maxRounds = Math.max(1, safeNumber(options.maxRounds, 3));
...
const repairGroups = getRepairGroups(buildingResults);
const positiveGroupSignals = new Set();
const attemptedGroupsByRound = new Map();
for (let round = 1; round <= maxRounds; round += 1) {
if (stoppedByTime) break;
const roundValueBefore = bestValue;
const roundAttempted = new Set();
attemptedGroupsByRound.set(round, roundAttempted);
const previousRoundImprovements = stats.improvedByGroups
.filter((item) => item.round === round - 1)
.map((item) => item.group);
const roundGroups = repairGroups.filter((group) => {
const signature = repairGroupSignature(group);
if (round > 1 && previousRoundImprovements.length && !previousRoundImprovements.some((improvedGroup) => repairGroupsOverlap(group, improvedGroup))) {
stats.skippedGroups.push({ round, group, reason: "no_overlap_with_previous_improvement" });
return false;
}
if (group.length >= 3) {
const hasPairSignal = repairGroups
.filter((candidate) => candidate.length === 2 && candidate.every((building) => group.includes(building)))
.some((pair) => positiveGroupSignals.has(repairGroupSignature(pair)));
if (!hasPairSignal) {
stats.skippedGroups.push({ round, group, reason: "no_positive_pair_signal" });
return false;
}
}
if (round > 1 && roundAttempted.has(signature)) return false;
return true;
});
const roundStats = {
round,
valueBefore: roundValueBefore,
valueAfter: roundValueBefore,
attempts: 0,
accepted: 0,
bestDelta: 0,
candidateExpansions: 0,
...
if (stoppedByTime) {
stats.skippedGroups.push({ round, group, reason: "time_limit" });
roundStats.completed = false;
continue;
}
stats.attempts += 1;
roundStats.attempts += 1;
roundAttempted.add(repairGroupSignature(group));
const valueBeforeGroup = bestValue;
...
roundStats.candidateExpansions += groupResult.candidateExpansions;
roundStats.feasibleCandidates += groupResult.feasibleCandidates;
const accepted = groupResult.value > bestValue;
let acceptedDelta = 0;
const bestDelta = Math.max(0, groupResult.value - valueBeforeGroup);
if (bestDelta > 0) {
positiveGroupSignals.add(repairGroupSignature(group));
stats.positiveSignals.push({ round, group, delta: bestDelta });
}
if (accepted) {
acceptedDelta = groupResult.value - bestValue;
...
stats.accepted += 1;
roundStats.accepted += 1;
stats.bestRepairDelta = Math.max(stats.bestRepairDelta, acceptedDelta);
roundStats.bestDelta = Math.max(roundStats.bestDelta, acceptedDelta);
stats.improvedByGroups.push({ round, group, delta: acceptedDelta, valueBefore: valueBeforeGroup, valueAfter: bestValue });
}
stats.groups.push({
round,
group,
valueBefore: valueBeforeGroup,
valueAfter: accepted ? bestValue : valueBeforeGroup,
...
roundStats.valueAfter = bestValue;
roundStats.totalDelta = bestValue - roundValueBefore;
stats.rounds.push(roundStats);
if (roundStats.accepted === 0) break;
}
stats.stoppedByTime = stoppedByTime;
stats.source = options.source || "best";
stats.sourceRank = options.sourceRank || 0;
...
function buildBeamStateResult(model, state, baseResult, sourceRank = 0) {
return buildResultFromSell(
model,
state.selection || new Map(),
state.selectedCombinations || new Map(),
baseResult.solverStatus,
{
...baseResult.stats,
archiveSource: {
source: "final_top_state",
sourceRank,
value: state.value || 0,
signature: state.archiveSignature || state.signature || ""
}
},
baseResult.runtimeMs,
baseResult.stoppedByTime
);
}
function selectArchiveRepairCandidates(topStates, finalResult, limit = 12) {
const candidates = (topStates || [])
.filter((state) => state?.selection && (state.value || 0) > 0)
.sort((a, b) => (b.value || 0) - (a.value || 0));
const selected = new Map();
function addCandidate(state, reason) {
if (!state || selected.size >= limit) return;
const signature = state.archiveSignature || probeStateArchiveSignature(state);
if (selected.has(signature)) return;
selected.set(signature, { state, reason });
}
for (const state of candidates.slice(0, Math.min(5, candidates.length))) {
addCandidate(state, "top_value");
}
const bestByUsage = new Map();
for (const state of candidates) {
const signature = usageProfileSignature(state);
const current = bestByUsage.get(signature);
if (!current || (state.value || 0) > (current.value || 0)) bestByUsage.set(signature, state);
}
for (const state of Array.from(bestByUsage.values()).sort((a, b) => (b.value || 0) - (a.value || 0))) {
addCandidate(state, "usage_diversity");
}
const timeLimit = getTimeLimitMinutes(finalResult?.settings || {});
const bySlack = [...candidates].sort((a, b) => {
function slackScore(state) {
let score = 0;
for (const [building, slots] of (state.buildingSlots || new Map()).entries()) {
score += Math.max(0, getSlotsForBuilding(building, finalResult?.settings || {}) - slots) * 10;
}
for (const [, minutes] of (state.buildingMinutes || new Map()).entries()) {
score += Math.max(0, timeLimit - minutes) * 0.01;
}
return score;
}
return slackScore(b) - slackScore(a) || (b.value || 0) - (a.value || 0);
});
for (const state of bySlack) {
addCandidate(state, "resource_slack");
}
return Array.from(selected.values());
}
function runArchiveRepair(model, buildingResults, finalResult) {
const candidates = selectArchiveRepairCandidates(finalResult.topStates, { settings: model.settings }, 12);
const stats = {
...
let bestResult = runLocalRepair(model, buildingResults, finalResult, { source: "best", sourceRank: 0 });
const baseBestValue = bestResult.objectiveValue;
for (const [index, candidate] of candidates.entries()) {
const state = candidate.state;
if ((state.value || 0) >= finalResult.objectiveValue && index === 0) continue;
const candidateBase = buildBeamStateResult(model, state, finalResult, index + 1);
const repairedCandidate = runLocalRepair(model, buildingResults, candidateBase, {
maxRuntimeMs: Math.max(350, Math.min(1100, Math.floor(Math.max(safeNumber(model.settings.maxRuntimeMs, 3500), 250) * 0.07))),
maxRounds: 2,
source: "final_top_state",
sourceRank: index + 1
});
stats.attempted += 1;
const deltaVsBaseBest = repairedCandidate.objectiveValue - baseBestValue;
stats.results.push({
sourceRank: index + 1,
reason: candidate.reason,
startValue: state.value || 0,
repairedValue: repairedCandidate.objectiveValue,
deltaVsBaseBest,
accepted: repairedCandidate.objectiveValue > bestResult.objectiveValue
});
if (repairedCandidate.objectiveValue > bestResult.objectiveValue) {
stats.accepted += 1;
stats.bestDelta = Math.max(stats.bestDelta, repairedCandidate.objectiveValue - bestResult.objectiveValue);
bestResult = repairedCandidate;
}
}
bestResult.stats.archiveRepair = stats;
return bestResult;
}
...
const probeSettings = {
...model.settings,
maxCombinationsPerBuilding: Math.min(Math.max(safeNumber(model.settings.maxCombinationsPerBuilding, 24), 1), 18)
};
...
beamWidth: Math.max(40, Math.min(120, Math.floor(beamWidth * 0.45))),
maxRuntimeMs: Math.max(800, Math.min(2200, Math.floor(maxRuntimeMs * 0.16))),
topStateArchiveLimit: 50,
phase: "probe"
});
...
const repairedResult = runArchiveRepair(model, buildingResults, finalResult);
return {
...
trimFeedback: repairedResult.stats.trimFeedback,
topStateArchive: finalResult.topStateArchiveDebug,
...
archiveRepair: repairedResult.stats.archiveRepair,
