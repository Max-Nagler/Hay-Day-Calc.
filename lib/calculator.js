...
function getQueueScore({ product, amount, mode }) {
  ...
  if (mode === "slots") {
    return amount * (product.timeMin || 0);
  }
...
function getBestProductForBuilding({
  products,
  building,
  indexes,
  settings,
  ...
  const warnings = [];
  const freeSlots = getFreeSlotsForBuilding(building);

  const candidates = products
    .filter((product) => getBuildingName(product) === building)
    .map((product) => {
      const baseAmount = getAmountForQueue(product, indexes, settings, freeSlots);

      const amount = getMaxMainAmountByIntermediateSlots({
        product,
        wantedAmount: baseAmount,
        indexes,
        settings,
        getFreeSlotsForBuilding
      });
...
function buildQueuePlan({ products, indexes, settings }) {
  ...
  for (const building of allBuildings) {
    const best = getBestProductForBuilding({
      products,
      building,
      indexes,
      settings,
    ...
    addUsedSlots(building, best.amount);
