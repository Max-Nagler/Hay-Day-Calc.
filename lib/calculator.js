...
function checkUsageFits(usedMinutesByBuilding, usedSlotsByBuilding, settings) {
  const reasons = [];
...
function buildBuildingCombinationDebug(building, candidates, best, indexes, settings) {
  ...
  const topCombinations = combinations
    ...
    .map((combination) => {
      ...
      const replacementState = createStateFromPicks(addCombinationPicks(replacementBasePicks, combination), candidates, settings);
      const globalCheck = replacementState
        ? checkUsageFits(replacementState.usedMinutesByBuilding, replacementState.usedSlotsByBuilding, settings)
        : {
            feasible: false,
            reasons: ["Als Ersatz für die aktuelle globale Lösung nicht möglich"],
            usage: []
          };
