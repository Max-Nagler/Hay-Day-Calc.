...
function getAllowedProducts({ products, settings }) {
  return products
    .filter((product) => isProductAllowed(product, settings))
...
function collectRequirements({
  ...
  if (depth > 20) {
    ...
    if (depth === 0) addToMap(directIngredientsMap, product.key, amount);
  ...
  for (const recipe of recipeRows) {
    ...
    const ingredientKey = recipe.ingredientKey || recipe.ingredient;
    const ingredientProduct = indexes.productsByKey.get(recipe.ingredientKey);

    if (depth === 0) {
      addToMap(directIngredientsMap, ingredientKey, ingredientAmount);
    }

    if (!ingredientProduct) {
      addToMap(ingredientsMap, ingredientKey, ingredientAmount);
...
function analyzeRequirements({ product, amount, indexes, settings, warnings }) {
  ...
  const directIngredientsMap = new Map();
  ...
  return {
    ingredientsMap,
    displayIngredientsMap: directIngredientsMap,
    intermediateMap,
    productionRequirements,
...
/**
 * Optimiert eine einzelne Warteschlange pro Gebäude.
 *
 * Regeln:
 * - Ein Gebäude produziert seriell.
 * - Gesamtzeit der gewählten Produkte darf die eingestellte Zeit nicht überschreiten.
 * - Anzahl der Aufträge darf die verfügbaren Slots nicht überschreiten.
 * - Verschiedene Produkte dürfen kombiniert werden.
 * - Bewertet wird die komplette Kombination:
 *   - Coins: maximaler Verkaufspreis gesamt
 *   - XP: maximale XP gesamt
 *   - Slots: maximale Zeitfüllung
 */
function optimizeBuildingQueue({
  products,
  building,
  indexes,
  settings,
  ...
  function stateKey(slotCount, timeUsed) {
    return `${slotCount}|${timeUsed}`;
  }
  ...
  bestByState.set(stateKey(0, 0), startState);
  ...
  const bestState = Array.from(bestByState.values())
    ...
    .sort((a, b) => {
      return b.score - a.score || b.slotCount - a.slotCount || b.timeUsed - a.timeUsed;
    })[0];
...
function buildQueuePlan({ products, indexes, settings }) {
  ...
  for (const building of allBuildings) {
    ...
    for (const pick of optimizedPicks) {
      ...
      if (settings.intermediateMustBeProduced) {
        let requirementsFit = true;

        for (const [intermediateKey, neededAmount] of entry.productionRequirements.entries()) {
          const intermediateProduct = indexes.productsByKey.get(intermediateKey);
          if (!intermediateProduct) continue;

          const intermediateBuilding = getBuildingName(intermediateProduct);
          const freeSlots = getFreeSlots(intermediateBuilding);

          if (neededAmount > freeSlots) {
            requirementsFit = false;
            warnings.push(
              `${entry.product.name} übersprungen: ${neededAmount}× ${intermediateProduct.name} benötigt, aber nur ${freeSlots} freie Slots in ${intermediateBuilding}.`
            );
            break;
          }
        }
      ...
      if (settings.intermediateMustBeProduced) {
        ...
        for (const [intermediateKey, neededAmount] of entry.productionRequirements.entries()) {
          ...
          addUsedSlots(getBuildingName(intermediateProduct), neededAmount);
        }
      }
    }
  }
...
export function calculateProductionPlan({
  ...
  // alte Props, damit alte page.js-Versionen nicht direkt brechen
...
export function getAvailableBuildings(products, level = 999) {
  ...
  for (const product of products) {
    ...
    if (!building.iconUrl && product.buildingIconUrl) {
      building.iconUrl = product.buildingIconUrl;
    }

    if (!building.slots && product.buildingSlots) {
      building.slots = product.buildingSlots;
    }
  }
