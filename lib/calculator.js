...
function buildBuildingDebugComparison(building, candidates, best, indexes, settings) {
  const relevantCandidates = candidates
    ...
    .map((candidate) => {
      const buildingMinutes = candidate.capacityMinutesByBuilding.get(building) || 0;
      const score = candidate.score;
      const efficiency = score / Math.max(buildingMinutes, 1);
      ...
      return {
        ...
        reason: `${Math.round(score)} Wert / ${Math.round(buildingMinutes)} ${building}-Minuten`
      };
    })
    .sort((a, b) => b.efficiency - a.efficiency || b.score - a.score);

  const chosen = Array.from(best.picks.entries())
    .map(([key, amount]) => {
      const candidate = candidates.find((item) => item.product.key === key);
      if (!candidate || !candidate.capacityMinutesByBuilding.has(building)) return null;

      const buildingMinutes = (candidate.capacityMinutesByBuilding.get(building) || 0) * amount;
      const score = candidate.score * amount;

      return {
        product: candidate.product.name,
        amount,
        score,
        coins: candidate.coins * amount,
        xp: candidate.xp * amount,
        buildingMinutes,
        totalCapacityMinutes: candidate.totalCapacityMinutes * amount,
        intermediateCapacityMinutes: candidate.intermediateCapacityMinutes * amount,
        efficiency: score / Math.max(buildingMinutes, 1),
        intermediates: mapIntermediateNames(candidate.productionRequirements, indexes.productsByKey),
        reason: `${Math.round(score)} Wert bei ${Math.round(buildingMinutes)} ${building}-Minuten`
      };
    })
    .filter(Boolean);

  const totalScore = chosen.reduce((sum, item) => sum + item.score, 0);
  const totalMinutes = chosen.reduce((sum, item) => sum + item.buildingMinutes, 0);

  return {
    building,
    chosen,
    topAlternatives: relevantCandidates.slice(0, 10),
    totalScore,
    totalMinutes,
    capacityMinutes: getBuildingCapacityMinutes(building, settings),
    reason:
      chosen.length
        ? `Gewählt wurde die Kombination mit ${Math.round(totalScore)} Wert innerhalb von ${Math.round(totalMinutes)}/${Math.round(getBuildingCapacityMinutes(building, settings))} Minuten. Verglichen wird nach Wert pro belegter ${building}-Minute inklusive Zwischenprodukten.`
        : `Keine gewählte Produktion für ${building}.`
  };
}
...
function optimizeGlobally(products, indexes, settings) {
  ...
  return {
    ...
    debug: {
      ...
      buildingComparisons: [
        buildBuildingDebugComparison("Tortenofen", feasibleCandidates, best, indexes, settings)
      ]
    }
  };
}
