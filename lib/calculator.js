import {
  buildBuildingUsageFromSolution,
  buildOptimizationModel,
  convertSolutionToProductionPlan,
  solveOptimizationModel,
  validateSolution
} from "./optimizationSolver";
...
function createDefaultSettings(settings = {}) {
  ...
  return {
    ...
    optimizationMode: settings.optimizationMode || settings.solverMode || "fast",
    solverMode: settings.solverMode || settings.optimizationMode || "fast",
    beamWidth: settings.beamWidth,
    maxRuntimeMs: settings.maxRuntimeMs
  };
}
...
function checkUsageFits(usedMinutesByBuilding, usedSlotsByBuilding, settings, previousUsage) {
  const reasons = [];
  const previousMinutes = previousUsage?.usedMinutesByBuilding || new Map();
  const previousSlots = previousUsage?.usedSlotsByBuilding || new Map();
  ...
  const usage = Array.from(new Set([
    ...previousMinutes.keys(),
    ...previousSlots.keys(),
    ...usedMinutesByBuilding.keys(),
    ...usedSlotsByBuilding.keys()
  ])).map((building) => {
    const newMinutes = usedMinutesByBuilding.get(building) || 0;
    const newSlots = usedSlotsByBuilding.get(building) || 0;
    const capacityMinutes = getBuildingCapacityMinutes(building, settings);
    const slotCapacity = getSlotsForBuilding(building, settings);

    return {
      building,
      oldSlots: previousSlots.get(building) || 0,
      newSlots,
      slotCapacity,
      oldMinutes: previousMinutes.get(building) || 0,
      newMinutes,
      capacityMinutes,
      feasible: newSlots <= slotCapacity && newMinutes <= capacityMinutes,
      minutes: newMinutes,
      slots: newSlots
    };
  });
...
function checkCombinationFitsGlobally(combination, settings) {
  ...
  return checkUsageFits(usedMinutesByBuilding, usedSlotsByBuilding, settings);
}

function removeMainBuildingPicks(picks, building, candidates) {
  const nextPicks = new Map(picks);

  for (const [key] of picks.entries()) {
...
function addCombinationPicks(basePicks, combination) {
...
function buildBuildingCombinationDebug(building, candidates, best, indexes, settings) {
  ...
  const buildingCandidates = candidates
    .filter((candidate) => getBuildingName(candidate.product) === building)
    .map((candidate) => {
  ...
  const topCombinations = combinations
    ...
    .slice(0, 10)
    .map((combination) => {
      const replacementBasePicks = removeMainBuildingPicks(best.picks, building, candidates);
      const replacementPicks = addCombinationPicks(replacementBasePicks, combination);
      const replacementState = createStateFromPicks(replacementPicks, candidates, settings, { allowInvalid: true });
      const globalCheck = checkUsageFits(
        replacementState.usedMinutesByBuilding,
        replacementState.usedSlotsByBuilding,
        settings,
        {
          usedMinutesByBuilding: best.usedMinutesByBuilding,
          usedSlotsByBuilding: best.usedSlotsByBuilding
        }
      );
      ...
      return {
        ...
        replacementUsage: globalCheck.usage,
        reason: globalCheck.feasible
          ? `${Math.round(combination.coins)} Coins in ${combination.usedSlots}/${slotLimit} ${building}-Slots und ${Math.round(combination.totalMinutes)}/${Math.round(timeLimit)} ${building}-Minuten; als Ersatz global möglich`
          : `Als Ersatz global nicht möglich: ${globalCheck.reasons.join("; ")}`
      };
    });
...
function createStateFromPicks(picks, candidates, settings, options = {}) {
  const state = {
    score: 0,
    coins: 0,
    xp: 0,
  ...
  return {
    ...
    debug: {
      ...
      buildingComparisons: Array.from(
        ...
        .filter((building) => feasibleCandidates.some((candidate) => getBuildingName(candidate.product) === building))
...
function buildMilpQueuePlan({ products, recipes, settings }) {
  const model = buildOptimizationModel(products, recipes, settings);
  const solution = solveOptimizationModel(model);
  const validation = validateSolution(solution, model.products, settings);
  const converted = convertSolutionToProductionPlan(solution, model.products, settings, model);
  const buildingUsage = buildBuildingUsageFromSolution(solution, settings);

  return {
    productionPlan: converted.productionPlan.sort((a, b) => {
    ...
    warnings: validation.infeasibleReasons || [],
    materialFlow: converted.materialFlow,
    buildingUsage,
    debug: {
      solverStatus: solution.solverStatus,
      objectiveValue: solution.objectiveValue,
      variablesCount: solution.variablesCount,
      constraintsCount: solution.constraintsCount,
      raw: solution.raw || {},
      validation,
      combinationDebug: solution.combinationDebug,
      infeasibleReasons: solution.infeasibleReasons || validation.infeasibleReasons || [],
      rejected: model.rejected || [],
      chosen: converted.productionPlan
        .filter((entry) => entry.sellAmount > 0)
        .map((entry) => ({
          product: entry.product.name,
          amount: entry.sellAmount,
          reason: `MILP verkauft ${entry.sellAmount}× und produziert ${entry.amount}×`
        })),
      topCandidates: [],
      buildingUsage,
      materialFlow: converted.materialFlow,
...
export function calculateProductionPlan({
  ...
  const { productionPlan, warnings, debug, materialFlow, buildingUsage: solverBuildingUsage } = buildMilpQueuePlan({
  ...
  return {
    ...
    buildingUtilization: solverBuildingUsage || buildBuildingUtilization(productionPlan, settings),
