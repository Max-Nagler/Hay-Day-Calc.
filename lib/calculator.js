...
function buildBuildingCombinationDebug(building, candidates, best, indexes, settings) {
  ...
  const topCombinations = combinations
    ...
    .map((combination) => ({
      ...
      reason: `${Math.round(combination.coins)} Coins in ${combination.usedSlots}/${slotLimit} Slots und ${Math.round(combination.totalMinutes)}/${Math.round(timeLimit)} Minuten`
    }));
  ...
  return {
    ...
    chosenCombination: {
      ...
      usedSlots: chosenOrders.length,
      slotCapacity: slotLimit,
      totalMinutes: chosenMinutes,
      capacityMinutes: timeLimit,
      coinsPerSlot: chosenCoins / Math.max(chosenOrders.length, 1)
    },
    topCombinations,
