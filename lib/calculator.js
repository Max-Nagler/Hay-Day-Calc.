...
function buildQueuePlan({ products, indexes, settings }) {
  ...
  if (settings.intermediateMustBeProduced) {
    ...
    for (const requirement of sortedRequirements) {
      ...
      if (freeSlots <= 0) {
        warnings.push(
          `Nicht genug freie Slots in ${building} für ${requirement.amount}× ${requirement.product.name}. Zwischenprodukt wird trotzdem als benötigte Produktion angezeigt.`
        );
      } else if (requirement.amount > freeSlots) {
        warnings.push(
          `${requirement.amount}× ${requirement.product.name} passt nicht in ${freeSlots} freie Slots von ${building}. Zwischenprodukt wird trotzdem als benötigte Produktion angezeigt.`
        );
      }
